...
function getAllPropValues(array, prop) {
  ...
  for (const itemArray of array) {
    ...
    for (const itemObject in itemArray) {
      // console.log(itemObject);
      if (itemObject === prop) {
        allProps.push(itemArray[prop]);
      }
    }
    // if (item.includes(prop)) {
    //   allProps.push(array[prop]);
    // }  Инклудс для объектов не использутся!!! Только для примитивных значений: строка, число, буль.
  }
  return allProps;
}
